...
function trimUnprocessableOptions(options) {
    if (options.packager.platform === 'win32' && !helpers_1.isWindows() && !helpers_1.hasWine()) {
...
async function buildNativefierApp(rawOptions) {
    ...
    if (isUpgrade(rawOptions)) {
        ...
        const oldApp = upgrade_1.findUpgradeApp(rawOptions.upgradeFrom);
        if (oldApp === null) {
    ...
    const appPathArray = await electron_packager_1.default(options.packager);
    log.info('\nFinalizing build...');
