...
async function userAgent(options) {
    ...
    options.packager.platform = optionsMain_1.normalizePlatform(options.packager.platform);
...
async function edgeUserAgent(platform, electronVersion) {
    const chromeVersion = await inferChromeVersion_1.getChromeVersionForElectronVersion(electronVersion);
...
async function firefoxUserAgent(platform) {
    const firefoxVersion = await inferFirefoxVersion_1.getLatestFirefoxVersion();
...
async function safariUserAgent(platform) {
    const safariVersion = await inferSafariVersion_1.getLatestSafariVersion();
